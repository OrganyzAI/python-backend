...
def authentication_token_from_email(client: TestClient, email: str, db: Session) -> dict[str, str]:
    ...
    if not user:
        ...
        user = User(email=email, hashed_password=hashed, first_name="Test", last_name="User")
        db.add(user)
        db.commit()
        db.refresh(user)

    token = security.create_access_token(subject=str(user.id), expires_delta=timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES))
    return {"Authorization": f"Bearer {token}"}


def get_superuser_token_headers(client: TestClient, db: Session) -> dict[str, str]:
    email = settings.FIRST_SUPERUSER if hasattr(settings, "FIRST_SUPERUSER") else "super@example.com"
    password = settings.FIRST_SUPERUSER_PASSWORD if hasattr(settings, "FIRST_SUPERUSER_PASSWORD") else "password"
    statement = select(User).where(User.email == email)
    user = db.exec(statement).first()
    if not user:
        hashed = security.get_password_hash(password)
        user = User(email=email, hashed_password=hashed, first_name="Super", last_name="User", role=UserRole.admin)
        db.add(user)
        db.commit()
        db.refresh(user)
    else:
